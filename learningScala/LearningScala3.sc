object LearningScala3 {
  ...
  "hola mundo".toUpperCase()                      //> res3: String = HOLA MUNDO
  def myToUpperCase(sentence: String): String = {
  	sentence.toUpperCase()
  }                                               //> myToUpperCase: (sentence: String)String
  val b = myToUpperCase("hola mundo")             //> b  : String = HOLA MUNDO
  
  def transformString(s: String, f: String => String) = {
  	f(s)
  }                                               //> transformString: (s: String, f: String => String)String
  
  val a = transformString("kuJikO", myToUpperCase)//> a  : String = KUJIKO
  println(a)                                      //> KUJIKO
  
  val c = transformString("pedro", x => x.toUpperCase())
                                                  //> c  : String = PEDRO
}